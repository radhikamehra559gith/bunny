...
# Load main DB credentials from env or local file
...
# Load all user DBs dynamically
user_dbs = {}
db_doc = main_db.collection("00-databses").document("Firebase").get()
if db_doc.exists:
    db_data = db_doc.to_dict()
    for key, val in db_data.items():
        user_dbs[key] = init_db_from_json(val, key)
        print(f"✅ Initialized DB client: {key}")
else:
    print("⚠️ No 'Firebase' doc found in '00-databses/Firebase'")
